 ...
 */
export function hasRefreshFailureBeenNotified(): boolean {
 ...
 */
export function markRefreshFailureNotified(): void {
 ...
 */
export function refreshAccessToken(client: AxiosInstance): Promise<void> {
  const promise = client
    .post<ApiResponse<unknown>>(API_ROUTES.auth.refresh)
    .then((response: AxiosResponse<ApiResponse<unknown>>): void => {
      if (response.data?.success === false) {
        throw new Error(
          response.data.message ?? "Refresh response indicated failure",
        );
      }
    })
 ...
 */
export function getOrCreateRefreshPromise(
